...
export const getWeatherForecast = async (lat: number, lon: number) => {
  const url =
    ...
    `&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code&timezone=auto`;
  ...
  return {
    ...
    today: {
      ...
      humidity: Math.round(data.current.relative_humidity_2m),
      wind: Math.round(data.current.wind_speed_10m),
      visibility: Math.floor(Math.random() * 4) + 6
    }
  };
};
